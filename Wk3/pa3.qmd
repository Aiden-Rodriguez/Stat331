...
```{r mutate-total}
colleges_clean <- colleges_clean |> 
    mutate(TOTAL_IN = UGDS * TUITIONFEE_IN)
```
...
```{r identify-college}
final_colleges <- colleges_clean |> 
  filter(REGION == 4, 
         ADM_RATE > median(ADM_RATE[REGION == 4], na.rm = TRUE), 
         TUITIONFEE_OUT > TUITIONFEE_IN, UGDS > 10000, 
         TOTAL_IN > 200000000, 
         SAT_AVG %% 2 != 0) |> 
  slice_min(TUITIONFEE_IN)
  # do later
